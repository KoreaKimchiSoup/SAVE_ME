...
public class TimeManager : MonoBehaviour
{
    ...
    // 시간을 5시간 뒤로 설정하는 함수
    public void AddFiveHours()
    {
        currentTime = currentTime.AddHours(5);

        // 만약 currentTime의 시간이 24시간 이상일 경우 날짜를 하루 더함
        if (currentTime.TimeOfDay.TotalHours >= 24)
        {
            currentTime = currentTime.AddDays(1).Date + currentTime.TimeOfDay;
